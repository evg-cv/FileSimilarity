...
from settings import INPUT_EXCEL_PATH, OUTPUT_DIR, SHEET_NAME
...
class FileSimilarity:
    ...
    def run(self):
        ...
        output_file_path = os.path.join(OUTPUT_DIR, f"{file_name}_{SHEET_NAME}_result.csv")
        ...
        if statements:
            ...
            output_df.to_csv(output_file_path, index=True, header=["Master Key", "Statements", "Proximity Score"],
                             mode='w')
            print(f"[INFO] Successfully saved in {output_file_path}")
        else:
            print(f"[INFO] There are not any statements to estimate in {SHEET_NAME}")
